...
public class Polygon2D implements Shape, Cloneable, Serializable {
    ...
    public void addPoint(float x, float y) {
        ...
        if(coordinates.size() >3){
            Coordinate[] coordArr = new Coordinate[coordinates.size()+1];
            coordArr = coordinates.toArray(coordArr);

            coordArr[coordinates.size()] = coordArr[0];

            CoordinateSequence coordinateSeq = new CoordinateArraySequence(coordArr);

            pointsRing = new LinearRing(coordinateSeq, geometryFactory);      
            geometry = new com.vividsolutions.jts.geom.Polygon(pointsRing, null, geometryFactory);
        }
    }
    ...
    public boolean contains(tool.Point p){
        return contains(p.getLat(), p.getLon());
    }

    public tool.Point getClosestPointToPoint(tool.Point point){
        tool.Point result = new tool.Point();
        
        Coordinate c1 = new Coordinate(point.getLat(), point.getLon());
        com.vividsolutions.jts.geom.Point p1 = geometryFactory.createPoint(c1);
        
        DistanceOp calculator = new DistanceOp(geometry, p1);
        Coordinate[] closestPt = calculator.closestPoints();

        result.setLat(closestPt[0].x);
        result.setLon(closestPt[0].y);
        
        return result;
    }
    
    public boolean isWithinDistance(tool.Point point, double distance){
        
        boolean result = false;
        
        if(!contains(point)){
            tool.Point closestPointInPolygon = getClosestPointToPoint(point);        
            result = LocationFunction.haversineDistance(point, closestPointInPolygon) < distance;
        }
        return result;
    }
